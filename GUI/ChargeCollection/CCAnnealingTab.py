...
from tab_config import CHARGE_COLLECTION_ANNEALING_TAB


class CCAnnealingTab(QWidget):
    def __init__(self):
        super().__init__()

        # Create widget with TabTemplate
        self.tab_template = TabTemplate(
            parent_tab=self, tab_config=CHARGE_COLLECTION_ANNEALING_TAB, 
            plot_function=self.plot_CC_annealing
            )
