...
from tab_config import CHARGE_COLLECTION_FLUENCE_TAB


class CCFluenceTab(QWidget):
    def __init__(self):
        super().__init__()
        
     # Create widget with TabTemplate
        self.tab_template = TabTemplate(
            parent_tab=self, tab_config=CHARGE_COLLECTION_FLUENCE_TAB, 
            plot_function=self.plot_CC_fluence
            )
    ...
    def plot_CC_fluence(self):
        # Load the DataFrame from the pickle file
        db_path = DEFAULT_DATABASE_PATH
        database = pd.read_pickle(db_path)
        
        save_path_dir = self.tab_template.save_path_input.text()
